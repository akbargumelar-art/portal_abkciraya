...
import { getStockPerdanaSummary, getStockPerdanaDetail } from '../services/api';
import { StockPerdanaData, StockPerdanaDetailData } from '../types';
...
// --- MAIN PAGE COMPONENT ---
const StockPerdanaPage: React.FC = () => {
    // State for summary dashboard
    const [summaryData, setSummaryData] = useState<StockPerdanaData[]>([]);
    ...
    const [detailData, setDetailData] = useState<StockPerdanaDetailData[]>([]);
    ...
    useEffect(() => {
        const fetchSummaryData = async () => {
            ...
            try {
                const summary = await getStockPerdanaSummary();
        ...
        const fetchDetailData = async () => {
            ...
            setErrorDetail(null);
            try {
                const detail = await getStockPerdanaDetail();
    ...
    const groupedSummaryData = useMemo(() => {
        return summaryData.reduce((acc, item) => {
            ...
            acc[tap].push(item);
            return acc;
        }, {} as Record<string, StockPerdanaData[]>);
    }, [summaryData]);
    ...
    const calculateTotals = (items: StockPerdanaData[]) => {
        const totals = items.reduce((acc, item) => {
            (Object.keys(item) as Array<keyof StockPerdanaData>).forEach(key => {
    ...
    const renderSummaryDataRow = (item: Partial<StockPerdanaData>, isTotal = false, tapName = '') => {
    ...
    // --- DETAIL TABLE LOGIC ---
    type ProcessedDetailData = StockPerdanaDetailData & { simpatiGrowth: number; byuGrowth: number; status: string; };
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Detail Stock Perdana");
        const date = new Date().toISOString().slice(0, 10);
        XLSX.writeFile(workbook, `detail_stock_perdana_${date}.xlsx`);
    };

    if (isLoadingSummary || isLoadingDetail) return <LoadingIndicator type="table" tableColumns={8} text="Memuat data stok perdana..." />;
    ...
    return (
        <div className="space-y-8">
            <div>
                 <h1 className="text-3xl font-bold text-gray-800">Stock Perdana</h1>
                 {lastUpdated && (
            ...
            <CollapsibleSection title="Detail Data Stock Perdana">
...
export default StockPerdanaPage;
