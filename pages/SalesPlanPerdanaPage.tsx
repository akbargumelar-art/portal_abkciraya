...
const SalesPlanPerdanaPage: React.FC = () => {
    const [data, setData] = useState<SalesPlanPerdanaData[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const result = await getSalesPlanPerdanaData();
                setData(result);
    ...
    const summaryData = useMemo<SummaryRow[]>(() => {
        ...
        const createSummaryMetric = (outlets: SalesPlanPerdanaData[], productSelector: (d: SalesPlanPerdanaData) => SalesPlanProductData, isQty: boolean): SummaryMetric => {
        ...
        for (const tap of sortedTaps) {
            ...
            const sfRows: SummaryRow[] = Object.entries(groupedBySf).map(([sfName, outlets]) => {
                ...
                return {
                    ...
                    oaTotal: createSummaryMetric(outlets, d => d.TotalPerdana, false),
                    qtyTotal: createSummaryMetric(outlets, d => d.TotalPerdana, true),
                };
            ...
            processedList.push({
                ...
                oaTotal: createSummaryMetric(outletsInTap, d => d.TotalPerdana, false),
                qtyTotal: createSummaryMetric(outletsInTap, d => d.TotalPerdana, true),
            });
        }
        ...
        processedList.push({
            ...
            oaTotal: createSummaryMetric(baseFilteredData, d => d.TotalPerdana, false),
            qtyTotal: createSummaryMetric(baseFilteredData, d => d.TotalPerdana, true),
        });
    ...
    type ProcessedData = SalesPlanPerdanaData & {
    ...
    const processedData = useMemo((): ProcessedData[] => {
        ...
        return baseFilteredData.map(item => {
            ...
            const totalData = getPeriodData(item.TotalPerdana);
    ...
    const columns = useMemo<ColumnDef<ProcessedData>[]>(() => [
        ...
        { accessorKey: 'totalTarget', header: 'Target', group: 'Total Perdana', align: 'right', cell: (row) => row.totalTarget.toLocaleString(), enableSorting: true },
        { accessorKey: 'totalM1', header: 'M-1', group: 'Total Perdana', align: 'right', cell: (row) => row.totalM1.toLocaleString(), enableSorting: true },
        { accessorKey: 'totalM', header: 'M', group: 'Total Perdana', align: 'right', cell: (row) => row.totalM.toLocaleString(), enableSorting: true },
        { accessorKey: 'totalAchv', header: 'Achv', group: 'Total Perdana', align: 'right', cell: (row) => <AchvCell value={row.totalAchv} />, enableSorting: true },
        { accessorKey: 'totalMom', header: 'MoM', group: 'Total Perdana', align: 'right', cell: (row) => <GrowthCell value={row.totalMom} />, enableSorting: true },
    ], []);
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Detail Sales Plan Perdana");
        const date = new Date().toISOString().slice(0, 10);
        XLSX.writeFile(workbook, `detail_sales_plan_perdana_${date}.xlsx`);
    };
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-gray-800">Sales Plan Perdana</h1>
                {lastUpdated && (
            ...
            <CollapsibleSection title="Detail Sales Plan Perdana">
...
export default SalesPlanPerdanaPage;
